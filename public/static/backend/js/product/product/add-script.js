...
var productAdd = {
    ...
    chooseAttribute: function (t) {
        ...
        $.getJSON(laroute.route('product.validation'), function (json) {
            ...
            if (jump.val() == '' || parseFloat(jump.val()) == 0.0) {
                flag = false;
                error_jump.text(json.product.enter_jump);
            } else {
                if (parseFloat(jump.val()) > parseFloat(max.val())) {
                    flag = false;
                    error_jump.text(json.product.enter_jump);
                } else {
                    error_jump.text('');
        ...
        $.getJSON(laroute.route('product.validation'), function (json) {
            ...
            if (form.valid()) {
                ...
                $('#tb-list-attribute tbody tr').each(function () {
                    ...
                    var error_min = $(this).find('.input_min').parents('tr').find('span');
                ...
                if (flag == true) {
                    if (arrayAttribute.length > 0) {
                        ...
                        $.ajax({
                            url: laroute.route('product.product.store'),
                            method: "POST",
                            data: {
