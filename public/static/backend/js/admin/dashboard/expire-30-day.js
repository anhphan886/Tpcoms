...
var KTAppExpire30DayDatatable = function () {
    // variables
    var datatable;
    var heading;
    ...
    var init = function () {
        ...
        datatable = $('#datatable_list_expire_30_day').KTDatatable({
            data: {
                type: 'remote',
                source: {
                    read: {
                        method: 'POST',
                        headers: {},
                        url: laroute.route('admin.dashboard.get-list-service-expire-day'),
                        params: {
                          day: 30
                        },
                        map: function(raw) {
...
KTUtil.ready(function () {
    KTAppExpire30DayDatatable.init();
    $('#search_expire_not_canceled').keypress(function () {
        $.ajaxSetup({ global: false });
    });
});

var functionClickTab = {
    tab_1:function () {
        functionClickTab.tab_general("search_expire_not_canceled");
    },
    tab_2:function () {
        functionClickTab.tab_general("search_expire_to_day");
    },
    tab_3:function () {
        functionClickTab.tab_general("search_expire_7_day");
    },
    tab_4:function () {
        functionClickTab.tab_general("search_expire_30_day");
    },
    tab_general: function(input_id) {
        $('#search_expire_not_canceled').keyup(function () {
            $.ajaxSetup({global: true});
        });
        $('#search_expire_to_day').keyup(function () {
            $.ajaxSetup({global: true});
        });
        $('#search_expire_7_day').keyup(function () {
            $.ajaxSetup({global: true});
        });
        $('#search_expire_30_day').keyup(function () {
            $.ajaxSetup({global: true});
        });
