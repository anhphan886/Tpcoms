...
@section('content')
    ...
    <form action="{{ route('ticket.queue.store') }}" method="POST" id="form-submit-queue">
        {{ csrf_field() }}
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">
                    @lang('ticket::queue.index.create-queue')
                </h3>
            ...
            <div class="kt-subheader__toolbar">
                <button type="button" class="btn btn-info btn-bold"  onclick="objQueue.save()">
                    @lang('ticket::queue.input.button_save_create')
                </button>
                <button type="button" class="btn btn-info btn-bold"  onclick="objQueue.save(1)">
                    @lang('ticket::queue.input.button_save_exit')
                </button>
...
{{--                                <input type="text" id="description" name="description" class="form-control">--}}
                            ...
                            <div class="form-group">
                                <label class="col-form-label">@lang('ticket::queue.index.email-password')</label>
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control ss-select-2"  id="ss-select-2" name="staff_operator[]" multiple="multiple">
                                            <optgroup>
                                                @if (isset($listAdmin))
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control ss-select-2"  id="ss-select-3" name="staff_processor[]" multiple="multiple">
                                        <optgroup>
                                            @if (isset($listAdmin))
                                                @foreach( $listAdmin as $item)
                                                    <option value="{{$item['id']}}">{{ $item['full_name'] }}</option>
...
@section('after_script')
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            $('#ss-select-2').select2({
                placeholder: "Chọn nhân viên chủ trì",
            });
