...
@section('content')
    <div class="kt-subheader kt-grid__item" id="kt_subheader">
        <div class="kt-subheader__main">
            <h3 class="kt-subheader__title">
                @lang('product::order.index.edit_order_adjust')
            </h3>
            ...
            <div class="kt-portlet__body">
                <form id="form-submit">
                    ...
                    <div class="kt-portlet__body">
                                ...
                                <select class="form-control --select2"
                                    ...
                                    @foreach($arrOption['arrCustomer'] as $customer)
                                        <option
                                            ...
                                            {{$orderDetail['customer_id'] == $customer['customer_id'] ? 'selected' : ''}}>
                                ...
                                <select class="form-control --select2 ss--width-100"
                                    ...
                                    @foreach($arrOption['arrStaff'] as $staff)
                                        <option value="{{$staff['id']}}"
                                            {{$orderDetail['staff_id'] == $staff['id'] ? 'selected' : ''}}>
                            ...
                            <div class="col-lg-4">
                                <select class="form-control --select2"
                                    ...
                                    @foreach($customerService as $service)
                                        @if($service['type'] == 'real')
                                            <option
                                                value="{{$service['customer_service_id']}}"
                                                {{$orderDetail['customer_service_id'] == $service['customer_service_id'] ? 'selected' : ''}}>
                                                {{$service['product_name_vi']}}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            @if($orderDetail['order_status_id'] == '1' )
                                <div class="col-lg-2">
                                ...
                                <div class="col-lg-4">
                                    <select class="form-control --select2" id="order_status_id" name="order_status_id">
                                        <option
                                            value="{{$orderDetail['order_status_id']}}">{{$orderDetail['order_status_name_vi']}}</option>
                        ...
                        <div class="form-group row">
                            <div class="col-lg-6">
                                <label class="col-form-label ">Nội dung tư vấn đơn hàng:</label>
                                <textarea name="order_content" id="order_content" rows="5"
                                          class="form-control">{{$orderDetail['order_content']}}</textarea>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                <div class="kt-portlet__head">
                    ...
                    <div class="kt-portlet__head-toolbar">
                        <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                            <a href="javascript:void(0)" onclick="orderAdjust.adjust()"
    ...
    <input type="hidden" id="order_code" value="{{$orderDetail['order_code']}}">
@endsection
@section('after_style')
...
@section('after_script')
    ...
    <script type="text/javascript"
            src="{{ asset('static/backend/js/product/order/edit-adjust.js?v='.time()) }}"></script>
@endsection
